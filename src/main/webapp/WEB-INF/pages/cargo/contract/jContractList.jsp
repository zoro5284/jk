...
<table id="ec_table" class="tableRegion" width="98%" >
	...
	<tr class="odd" onmouseover="this.className='highlight'" onmouseout="this.className='odd'" >
		...
		<td>${o.customName}</td>
		<td><a href="toview.action?id=${o.id}">${o.contactNo}</a></td>
		<td>${o.inputBy}</td>
		<td>${o.checkBy }</td>
		<td>${o.inspector }</td>
		<td>${o.singingDate }</td>
		...
		<td>${o.state }</td>
		<td>
		<c:if test="${o.state==1}"><a href="cancel.action?id=${o.id }"><font color="green">取消</font></a></c:if>
		<c:if test="${o.state==0}"><a href="submit.action?id=${o.id }">上报</a></c:if>
		</td>
	</tr>
	</c:forEach>
